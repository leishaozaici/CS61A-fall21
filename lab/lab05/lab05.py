...
def flatten(s):
    ...
    # return [s[0]] + flatten(s[1:]) if s and type(s[0]) != list else flatten(s[0]) + flatten(s[1:]) if type(s[0]) == list and s else []
...
def distance(city_a, city_b):
    ...
    """
    "*** YOUR CODE HERE ***"
    lat_a,lot_a = get_lat(city_a),get_lon(city_a)
    lat_b,lot_b = get_lat(city_b),get_lon(city_b)
    return sqrt((lat_a -lat_b) ** 2 + (lot_a -lot_b) ** 2)
...
def closer_city(lat, lon, city_a, city_b):
    ...
    """
    "*** YOUR CODE HERE ***"
    return get_name(city_a) if distance(make_city("x",lat,lon),city_a) < distance(make_city("x",lat,lon),city_b) else get_name(city_b)
