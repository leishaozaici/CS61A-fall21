...
def about(topic):
    ...
    "*** YOUR CODE HERE ***"
    def select(pagraphs):
        temp_list = split(remove_punctuation(lower(pagraphs)))
        for i in topic:
            if temp_list.count(lower(i)) > 0:
                return True
        return False
    return select
    # END PROBLEM 2
...
def accuracy(typed, reference):
    ...
    # BEGIN PROBLEM 3
    "*** YOUR CODE HERE ***"
    if (not typed_words and reference_words) or (typed_words and not reference_words):
        return 0.0
    if typed_words == reference_words[:len(typed_words)]:
        return 100.0
    common_len = min(len(typed_words), len(reference_words))
    return [x == y for x, y in zip(typed_words[:common_len], reference_words[:common_len])].count(True) / max(len(typed_words), len(reference_words)) * 100
...
def autocorrect(typed_word, valid_words, diff_function, limit=10):
    ...
    if typed_word in valid_words:
        return typed_word
    min_diff = min([diff_function(typed_word, y, limit)] for y in valid_words)
    for i in valid_words:
        temp_diff = diff_function(typed_word, i, limit)
        if temp_diff == min_diff[0] and temp_diff <= limit:
            return i
    return typed_word
    # END PROBLEM 5


def diff_function(w1, w2, limit): return abs(len(w2) - len(w1))


print(autocorrect("cul", ["culture", "cult", "cultivate"], diff_function, 10))
...
def feline_flips(start, goal, limit):
    ...
    # if len(start) == len(goal) == 1 and start[0] != goal[0]:
    #     return 1
    # if limit == 0:
    #     if start == goal:
    #         return 0
    ...
    #     diff_count = abs(len(goal)-len(start))
    #     avg_len = min(len(start), len(goal))
    #     return diff_count + feline_flips(start[:avg_len], goal[:avg_len], limit-1)
    # if len(start) == len(goal) and start and start[-1] != goal[-1]:
    ...
    if len(start) != len(goal):
        avg_len = min(len(start), len(goal))
        return abs(len(start)-len(goal)) + feline_flips(start[:avg_len], goal[:avg_len], limit - 1)
    return int(start[-1] != goal[-1]) + feline_flips(start[:-1], goal[:-1], limit - int(start[-1] != goal[-1]))

    # END PROBLEM 6
print(feline_flips("car", "cad", 10))
print(feline_flips("from", "form", 10))
...
def time_per_word(words, times_per_player):
    ...
    #     times.append([times_per_player[i][j] - times_per_player[i][j-1]
    #                  for j in range(1, len(words)+1)])

    # return match(words, times)

    return match(words, [[times_per_player[i][j]-times_per_player[i][j-1]for j in range(1, len(words)+1)] for i in range(0, len(times_per_player))])
    # END PROBLEM 9
...
def fastest_words(match):
    ...
    # BEGIN PROBLEM 10
    "*** YOUR CODE HERE ***"
    res = [[] * k for k in player_indices]
    for i in word_indices:
        times = [time(match, j, i) for j in player_indices]
        res[times.index(min(times))] += [word_at(match, i)]
    # END PROBLEM 10
    return res
