...
def play(strategy0, strategy1, score0=0, score1=0, dice=six_sided,
    ...
    # BEGIN PROBLEM 5
    "*** YOUR CODE HERE ***"
    """ max(strategy0(score0,score1),strategy1(score1,score0)) """
    while True:
        if score0 >= goal or score1 >= goal:
            break
        if who == 0:
            score0 += take_turn(strategy0(score0, score1),
                                score1, dice, GOAL_SCORE)
            score0 += hog_pile(score0, score1)
        else:
            score1 += take_turn(strategy1(score1, score0),
                                score0, dice, GOAL_SCORE)
...
def announce_highest(who, last_score=0, running_high=0):
    ...
    #         if running_temp > running:
    #             running = running_temp
    #             print(running,"point(s)! That's a record gain for Player ",who,"!")
    #     return announce_highest(who,score,running)
    # return say
    def say(score0, score1):
        if who == 0:
...
def max_scoring_num_rolls(dice=six_sided, trials_count=1000):
    ...
    num_rolls = 0
    for i in range(1,11):
        temp_score = avg(i,dice)
        if  temp_score > max_score:
            max_score , num_rolls = temp_score,i
    return num_rolls
...
def hog_pile_strategy(score, opponent_score, cutoff=8, num_rolls=6):
    ...
    if score + roll_0_score == opponent_score:
        return 0
